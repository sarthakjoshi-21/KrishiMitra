...
export default function FarmerLoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (!isLogin) {
        // Sign Up flow
        const result = await signUpUser(data, 'farmer')
        if (result.error) {
          if (result.error.includes('fetch') || result.error.includes('URL')) {
            router.push(`/?role=farmer&name=${encodeURIComponent(name)}`)
            return
          }
        ...
        router.push(`/?role=farmer&name=${encodeURIComponent(session.fullName || name)}`)
        return
      }
